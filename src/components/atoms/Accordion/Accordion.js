...
const Head = styled.div`
  display: flex; //para titulo a esquerda e icone a direita
  ...
  & h6 {
    margin: 0;
    flex: 1; //para o titulo usar todo o espaço
  }
...
//pOpen para definir se o component é controlado
//onChange notifica que o estad mudou
const Accordion = ({ title, open: pOpen, onChange, children }) => {
  ...
  const handleClick = () => {
    ...
    if (!isControlled) {
      setOpen(newState);
      //controla estado interno, por isso só dispara nesse caso
    }
  ...
  return (
    <Root open={open}>
      {/* role para avisar que a div tem uma propriedade como de botão */}
